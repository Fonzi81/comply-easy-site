-- Fix the admin_list_users function by casting email to text
...
begin
  ...
  -- Return users with their profiles using fully qualified column names and proper casting
  return query
    select 
      auth_users.id,
      auth_users.email::text,  -- Cast varchar(255) to text
      user_profiles.role,
